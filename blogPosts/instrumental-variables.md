...
The second assumption is known as _exclusion restriction_. These assumptions are reasonable in this setup, and this assumption eliminates the effect of the never takers from causal consideration. Techincally, it also eliminates always takers, so we can relax the one-sided noncompliance to just exclude defiers, which is known as _monotonicity_ . However, I've yet to think of a situation where taking out only defiers is natural. From a causality standpoint, this assumption is what allows to get back to the RCT framework, where the population consists only of compliants.
